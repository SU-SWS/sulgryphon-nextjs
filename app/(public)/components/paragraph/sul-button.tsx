...
const SulButton = ({paragraph, siblingCount = 0}: { paragraph: ButtonParagraph, siblingCount?: number }) => {
  const isGray = paragraph.behavior_settings?.sul_button_styles?.background == 'gray';
  return (
    <div className="su-w-full">
      <div className={(siblingCount > 0 ? "su-px-50 su-w-full ": "su-full-screen ") + " su-py-50 " + (isGray ? "su-bg-black-10" : "su-bg-black-true")}>
        <div className="su-cc">
          <Conditional showWhen={paragraph.sul_button_headline}>
            <h2 className={"su-text-center su-text-m3 " + (!isGray ? 'su-text-white' : '')}>{paragraph.sul_button_headline}</h2>
          </Conditional>
