...
const linkIsExternal = (url) => {
  return url && !(url.startsWith('/') || url.startsWith('https://library.stanford.edu'));
}

const massageHref = (url: string) => {
  return url.replace(process.env.NEXT_PUBLIC_DRUPAL_BASE_URL ?? '', '');
}

export const DrupalLinkButton = ({href, children, className = '', buttonProps = null, ...props}) => {
  href = massageHref(href);

  const Component = linkIsExternal(href) ? 'a': Link;
  return (
    <Component
      href={href}
      {...props}
